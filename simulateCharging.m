function [retentionRate, useRate, s_N, u_N] = simulateCharging(lambda, numChargers, serDesks, numSimulations)

    % 模拟参数
    % lambda  % 泊松分布参数
    % numChargers  % 初始充电桩数量
    % servuceDesks  % 初始换电台数量
    % numSimulations  % 模拟次数
    ...
    % 计算换电台使用率
    useRate = use_num / (numSimulations * serDesks);
end
